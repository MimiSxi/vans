...
if (!function_exists('AddOrderAction')) 
{
     ...
     * 参数说明：
     * $OrderId       订单ID或订单ID数组
     * $UsersId       用户ID，若不为0，则ActionUsers为0
     * $ActionUsers   操作员ID，为0，表示用户操作，反之则为管理员ID
     ...
     * return 无需返回
     */
    function AddOrderAction($OrderId,$UsersId,$ActionUsers='0',$OrderStatus='0',$ExpressStatus='0',$PayStatus='0',$ActionDesc='提交订单！',$ActionNote='用户提交订单成功！')
    {
...
if (!function_exists('GetUsersLatestData')) 
{
    ...
    function GetUsersLatestData($users_id = null) {
        ...
        if(!empty($users_id)) {
            ...
            /*用户数据处理*/
            // 头像处理
            $users['head_pic'] = get_head_pic(htmlspecialchars_decode($users['head_pic']));
            // 昵称处理
            ...
            if (intval($users['level_maturity_days']) >= 36600) {
                ...
                $users['maturity_date'] = '未升级用户';// 没有升级用户，置空
            }else{
                /*计算剩余天数*/
                ...
                if (0 >= $days) {
                    ...
                    $users['maturity_date'] = '未升级用户';// 用户过期，置空
                }else{
