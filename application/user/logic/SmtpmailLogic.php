    ...
    public function send_email($email = '', $title = '', $type = 'reg', $scene = 2, $data = [])
    {
        ...
        if ('retrieve_password' == $type) 
        {
            ...
            if (!empty($usersdata)) {
                if (empty($usersdata['is_activation'])) {
                    return ['code'=>0, 'msg'=>'该用户尚未激活，不能找回密码！'];
        ...
        else if ('order_msg' == $type) 
        {
            ...
            if (!empty($data)) {
                ...
                switch ($data['type']) {
                    case '1':
                        $content = '您好，管理员。 用户(' . $data['nickname'] . ')使用'. $PayMethod .'对订单(' . $data['order_code'] . ')支付完成，请登录后台审查并及时发货。';
                        break;
                    case '2':
                        $url = request()->domain() . url('user/Shop/shop_order_details', ['order_id'=>$data['order_id']]);
                        $chayue = '<a href="'. $url .'">查阅</a>';
                        $content = '您好，' . $data['nickname'] . '。 管理员已对订单(' . $data['order_code'] . ')发货完成，请登录用户中心'. $chayue .'。';
                        break;
                }
            }
        }
