...
class PayLogic extends Model
{
    ...
    public function initialize() {
        ...
        $this->users_type_manage_db  = Db::name('users_type_manage');   // 用户升级分类价格表
    }
            ...
            if (!empty($OrderData)) {
                ...
                if (0 == $OrderData['order_status']) {
                    ...
                    if (!empty($ReturnId)) {
                        ...
                        // 添加订单操作记录
                        AddOrderAction($OrderData['order_id'],$OrderData['users_id'],'0','1','0','1','支付成功！','用户使用支付宝完成支付！');
                        if (1 == $param['is_ailpay_notify']) {
                            return "success";
                        }else{
                            $msg = [
