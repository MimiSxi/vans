...
class Level extends Base {
    ...
    public function __construct(){
        ...
        $this->users_type_manage_db  = Db::name('users_type_manage');   // 用户等级表
        /*结束*/
    ...
    public function level_type_del()
    {
        ...
        if (IS_AJAX_POST && !empty($type_id)) {
            ...
            if ($return) {
                adminLog('删除用户升级级别：'.$type_name_list);
                $this->success('删除成功');
            }else{
                $this->error('删除失败');
            }
    ...
    public function add_level_data()
    {
        if (IS_POST) {
            ...
            foreach ($post['type_name'] as $key => $value) {
                ...
                if (empty($level_id))   $this->error('用户级别不可为空');
                if (empty($price))      $this->error('产品价格不可为空');
                if (empty($limit_id))   $this->error('用户期限不可为空');
