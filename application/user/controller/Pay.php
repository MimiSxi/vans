                    ...
                    if ($result['trade_state'] == 'SUCCESS' && !empty($result['transaction_id'])) {
                        if (2 == $transaction_type) {
                            ...
                            if (empty($order_data['order_status'])) {
                                ...
                                // 修改用户金额明细表中，对应的订单数据，存入返回的数据，订单已付款
                                $OrderData = [
                                    'order_status' => 1,
                                ...
                                if (!empty($order_id)) {
                                    ...
                                    // 添加订单操作记录
                                    AddOrderAction($order_data['order_id'],$this->users_id,'0','1','0','1','支付成功！','用户使用微信完成支付！');
        ...
        if (IS_AJAX_POST) {
            ...
            if ($this->users['users_money'] >= $Data['order_amount']) {
                ...
                if (!empty($return)) {
                    ...
                    if (!empty($users_id)) {
                        // 添加订单操作记录
                        AddOrderAction($post['unified_id'],$this->users_id,'0','1','0','1','支付成功！','用户使用余额完成支付！');
                        // 虚拟自动发货
                    ...
                    if (1 == $transaction_type) {
                        ...
                        if (0 == $order_data['order_status']) {
                            ...
                            if (!empty($order_id)) {
                                ...
                                // 添加订单操作记录
                                AddOrderAction($order_data['order_id'],$order_data['users_id'],'0','1','0','1','支付成功！','用户使用微信小程序完成支付！');
