...
use app\home\model\Productmanagement;
use app\home\model\Workmanagement;
use think\Page;
use think\Db;
...
class Custom extends Base
{
    ...
    public function add()
    {
        ...
        $workid = input('param.workid/d', 0);
        $assign_data['typeid'] = $typeid; // 栏目ID
        $assign_data['workid'] = $workid; // 作品ID
        // 栏目信息
...
//        $arctype_html = allow_release_arctype($typeid, array($this->channeltype));
    ...
    public function edit()
    {
        ...
        $list = Db::name('productmanagement_content')->where(['workid' => $id])->select();
        $assign_data['product_list'] = $list;
...
//
        ...
        if ($typeid == 80) {
            $inandout = Db::table('ey_inandout')->alias('a')->where(['a.productid' => $id])->select();
            $assign_data['inandout'] = $inandout;
        }
