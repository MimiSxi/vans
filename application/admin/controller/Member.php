...
class Member extends Base
{
    ...
    public function __construct()
    {
        ...
        $this->users_type_manage_db = Db::name('users_type_manage'); // 用户属性表
        /*结束*/
    ...
    // 检测并第一次从官方同步用户中心的前台模板
    public function ajax_syn_theme_users()
    {
        $msg = '下载用户中心模板包异常，请第一时间联系技术支持，排查问题！';
    ...
    public function users_edit()
    {
        if (IS_POST) {
            ...
            $mid = input('id/d');
            Db::name('users')->where('users_id', $mid)->update(['area_id' => $post['area_id']]);
        ...
        $id = input('id/d');
        $abc = Db::name('users')->where('users_id', $id)->find();
    ...
    public function level_add()
    {
        if (IS_POST) {
            ...
            foreach ($post['level_name'] as $key => $value) {
                ...
                if (empty($level_value)) $this->error('用户等级值不可为空！');
            ...
            if ($ReturnId) {
                adminLog('新增用户级别：' . implode(',', $post['level_name']));
