<?php if (!defined('THINK_PATH')) exit(); /*a:4:{s:47:"./application/admin/template/auth_role\edit.htm";i:1604970384;s:80:"C:\software\EyouCMS-V1.4.9-UTF8-SP2\application\admin\template\public\layout.htm";i:1596507477;s:82:"C:\software\EyouCMS-V1.4.9-UTF8-SP2\application\admin\template\admin\admin_bar.htm";i:1573115082;s:80:"C:\software\EyouCMS-V1.4.9-UTF8-SP2\application\admin\template\public\footer.htm";i:1571728724;}*/ ?>
<!doctype html>
<html>
<head>
...
<div class="page">
    ...
    <?php endif; ?>
    <form class="form-horizontal" id="postForm" method="post" action="<?php echo url('AuthRole/edit'); ?>">
        <div class="ncap-form-default">
            ...
            <dl class="row">
                ...
                <dd class="opt">
                    <input type="text" name="name" id="name" value="<?php echo (isset($info['name']) && ($info['name'] !== '')?$info['name']:''); ?>" class="input-txt">
                    <span class="err"></span>
                    <p class="notic"></p>
                </dd>
            </dl>
            ...
            <dl class="row">
                ...
                <dd class="opt">
                    ...
                    <p>
                        <label><input type="radio" name="tmp_role_id" value="<?php echo $role['id']; ?>" <?php if($info['id'] == $role['id']): ?>checked<?php endif; ?> /><?php echo $role['name']; ?></label>
                    </p>
            ...
            <dl class="row none">
                ...
                <dd class="opt">
                    <label><input type="checkbox" name="language[]" value="cn" <?php if(! empty($info['language']) && in_array('cn', $info['language'])): ?> checked="checked"<?php endif; ?> />简体中文</label>
                    <span class="err"></span>
                    <p class="notic"></p>
                </dd>
            </dl>
            <dl class="row">
                ...
                <dd class="opt">
                    <label><input type="checkbox" name="online_update" value="1"<?php if($info['online_update'] == '1'): ?> checked="checked"<?php endif; ?> />允许操作</label>
                    <span class="err"></span>
                    <p class="notic"></p>
                </dd>
            </dl>
            <dl class="row" id="dl_only_oneself">
                ...
                <dd class="opt">
                    <label><input type="checkbox" name="only_oneself" value="1"<?php if($info['only_oneself'] == '1'): ?> checked="checked"<?php endif; ?> />只允许查看自己发布的文档</label>
                    <span class="err"></span>
                    <p class="notic"></p>
                </dd>
            </dl>
            <dl class="row">
                ...
                <dd class="opt">
                    <p><label><input type="checkbox" id="select_cud"<?php if(! empty($info['cud']) && count($info['cud'])>=3): ?> checked="checked"<?php endif; ?> />完全控制</label></p>
                    <p><label><input type="checkbox" name="cud[]" value="add"<?php if(! empty($info['cud']) && in_array('add', $info['cud'])): ?> checked="checked"<?php endif; ?> />添加信息</label></p>
                    <p><label><input type="checkbox" name="cud[]" value="edit"<?php if(! empty($info['cud']) && in_array('edit', $info['cud'])): ?> checked="checked"<?php endif; ?> />修改信息</label></p>
                    <p><label><input type="checkbox" name="cud[]" value="del"<?php if(! empty($info['cud']) && in_array('del', $info['cud'])): ?> checked="checked"<?php endif; ?> />删除信息</label></p>
                    <p><label><input type="checkbox" name="cud[]" value="changetableval"<?php if(! empty($info['cud']) && in_array('changetableval', $info['cud'])): ?> checked="checked"<?php endif; ?> />审核信息</label></p>
                    <span class="err"></span>
                    <p class="notic"></p>
                </dd>
            </dl>
                ...
                <dd class="opt">
                            ...
                            <div class="admin_poplistdiv">
                                ...
                                <p>
                                    <?php $first_arctype_id = ''; if(is_array($arctypes) || $arctypes instanceof \think\Collection || $arctypes instanceof \think\Paginator): if( count($arctypes)==0 ) : echo "" ;else: foreach($arctypes as $k=>$arctype): if(isset($arctype_array[$arctype['id']])): if($k>0): ?>
                                        ...
                                        <label><input type="checkbox" class="arctype_cbox arctype_id_<?php echo $arctype['id']; ?>" name="permission[arctype][]" value="<?php echo $arctype['id']; ?>"<?php if(! empty($info['permission']['arctype']) && in_array($arctype['id'], $info['permission']['arctype'])): ?> checked="checked"<?php endif; ?> /><?php echo $arctype['typename']; ?></label>
                                    <?php endforeach; endif; else: echo "" ;endif; ?>
                                </p>
                                ...
                                <?php if(is_array($arctypes) || $arctypes instanceof \think\Collection || $arctypes instanceof \think\Paginator): if( count($arctypes)==0 ) : echo "" ;else: foreach($arctypes as $k=>$arctype): if(isset($arctype_array[$arctype['id']])): ?>
                                    ...
                                    <?php foreach($arctype_array[$arctype['id']] as $item): ?>
                                        <div class="arctype_child1">
                                            <label><input type="checkbox" class="arctype_cbox arctype_id_<?php echo $item['id']; ?>" name="permission[arctype][]" value="<?php echo $item['id']; ?>" data-pid="<?php echo $item['parent_id']; ?>" <?php if(! empty($info['permission']['arctype']) && in_array($item['id'], $info['permission']['arctype'])): ?> checked="checked"<?php endif; ?> /><?php echo $item['typename']; ?></label>
                                        </div>
                                        ...
                                        <div class="arctype_child2" id="arctype_child_<?php echo $item['id']; ?>">
                                            ...
                                            <label><input type="checkbox" class="arctype_cbox" name="permission[arctype][]" value="<?php echo $vo['id']; ?>" data-pid="<?php echo $vo['parent_id']; ?>" data-tpid="<?php echo $item['parent_id']; ?>" <?php if(! empty($info['permission']['arctype']) && in_array($vo['id'], $info['permission']['arctype'])): ?> checked="checked"<?php endif; ?> /><?php echo $vo['typename']; ?></label>
                                            <?php endforeach; ?>
                                        </div>
                          ...
                          <?php else: ?>
                            <div class="admin_poplistdiv">
                                ...
                                <p>
                                    ...
                                    <label><input type="checkbox" name="permission[rules][]" value="<?php echo $rule['id']; ?>" <?php if(! empty($info['permission']['rules']) && in_array($rule['id'], $info['permission']['rules'])): ?> checked="checked"<?php endif; ?> /><?php echo $rule['name']; ?></label>
                                    <?php endforeach; endif; else: echo "" ;endif; ?>
                                </p>
                            </div>
                    ...
                    <div class="admin_poplistdiv">
                        ...
                        <ul>
                            ...
                            <li>
                                <label><input type="checkbox" name="permission[plugins][<?php echo $plugin['code']; ?>][code]" value="<?php echo $plugin['code']; ?>" <?php if(!empty($info['permission']['plugins'][$plugin['code']]) && isset($info['permission']['plugins'][$plugin['code']]['child'])): ?> checked="checked"<?php endif; ?> /><?php echo $plugin['name']; ?></label>
                                <?php $config = json_decode($plugin['config'], true); if(! empty($config['permission'])): ?>
                                <p style="padding-left:10px;">
                                    <span class="button level1 switch center_docu"></span>
                                    <?php foreach($config['permission'] as $index => $text): ?>
                                    <label><input type="checkbox" name="permission[plugins][<?php echo $plugin['code']; ?>][child][]" <?php if(! empty($info['permission']['plugins'][$plugin['code']]['child']) && in_array($index, $info['permission']['plugins'][$plugin['code']]['child'])): ?> checked="checked"<?php endif; ?> value="<?php echo $index; ?>" /><?php echo $text; ?></label>
                                    <?php endforeach; ?>
                                </p>
                                <?php endif; ?>
                            </li>
...
<script type="text/javascript">
    ...
    function postSubmit(){
        ...
        <?php else: ?>
            $.ajax({
                url: "<?php echo url('AuthRole/edit', ['_ajax'=>1]); ?>",
                type: 'POST',
                dataType: 'JSON',
