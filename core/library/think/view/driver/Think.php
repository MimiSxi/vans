    ...
    private function parseTemplate($template)
    {
        ...
        if (0 !== strpos($template, '/')) {
            ...
            if (in_array($request->module(), ['admin']) && 'weapp' == strtolower($request->controller()) && 'execute' == strtolower($request->action())) {
                ...
                if ($controller) {
                    /*用户中心模板切换，兼容响应式模板目录下，users目录可以灵活定义wap手机端用户中心模板 by 小虎哥*/
                    if ('user' == $request->module()) {
                        ...
                        if (file_exists('./template/pc/'.$web_users_tpl_theme.'/'.$users_wap_tpl_dir) && !file_exists('./template/mobile') && isMobile()) {
