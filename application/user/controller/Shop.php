...
class Shop extends Base
{
        ...
        if (IS_AJAX_POST) {
            ...
            if (!empty($order_id)) {
                ...
                if (!empty($return)) {
                    ...
                    // 添加订单操作记录
                    AddOrderAction($order_id,$this->users_id,'0','0','0','0','订单取消！','用户关闭订单！');
                    $this->success('订单已取消！');
                }else{
                    $this->error('操作失败！');
                }
    ...
    public function shop_payment_page()
    {
        if (IS_POST) {
            ...
            foreach ($list as $key => $value) {
                ...
                // 若库存为空则返回提示
                if (empty($value['stock_count'])) $this->error('产品 《'.$value['title'].'》 库存不足！');
            ...
            if (!empty($OrderId)) {
                ...
                if (!empty($OrderId) && !empty($DetailsId)) {
                    ...
                    if (0 == $post['payment_method']) {
                        ...
                        // 货到付款时，再次添加一条订单操作记录
                        AddOrderAction($OrderId, $this->users_id, 0, 1, 0, 1, '货到付款！', '用户选择货到付款，款项由快递代收！');
    ...
    public function shop_order_remind()
    {
        if (IS_AJAX_POST) {
            $post = input('post.');
            // 添加订单操作记录
            AddOrderAction($post['order_id'],$this->users_id,'0','1','0','1','提醒成功！','用户提醒管理员及时发货！');
            $this->success('提醒成功！');
        }else{
            $this->error('订单号错误');
        }
    ...
    public function shop_member_confirm()
    {
        if (IS_AJAX_POST) {
            ...
            if (!empty($return)) {
                ...
                // 添加订单操作记录
                AddOrderAction($post['order_id'],$this->users_id,'0','3','1','1','确认收货！','用户已确认收到货物，订单完成！');
                $this->success('用户确认收货');
            }else{
                $this->error('订单号错误');
            }
