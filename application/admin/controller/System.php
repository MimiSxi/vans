...
class System extends Base
{
    ...
    public function web2()
    {
        ...
        if (IS_POST) {
            ...
            /*更改session用户设置 - session有效期（后台登录超时）*/
            $session_conf = [];
    ...
    public function send_mobile()
    {
        ...
        $send_scene = M('sms_template')->where(['sms_tpl_code'=>['NEQ', '']])->order('tpl_id asc')->value('send_scene');
        $send_scene = intval($send_scene);
        $res = sendSms($send_scene, $param['sms_test_mobile'], array('content'=>mt_rand(1000,9999)));
        if (intval($res['code']) == 1) {
            /*多语言*/
            if (is_language()) {
                $langRow = \think\Db::name('language')->order('id asc')
    ...
    public function ajax_tag_call()
    {
        ...
        if (IS_AJAX_POST) {
            ...
            $msg = '';
            switch ($name) {
                case 'web_users_switch': // 用户功能入口标签
                    {
$msg_code = <<<EOF
...
$tpl_theme = TPL_THEME;
$msg = <<<EOF
<strong>前台用户登录注册标签调用</strong><br>
比如需要在PC通用头部加入用户入口，复制下方代码在/template/{$tpl_theme}pc/header.htm模板文件里找到合适位置粘贴
<br/><br/>
<div style="color:red">
{$msg_code}
</div>
