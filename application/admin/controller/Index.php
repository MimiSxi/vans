...
class Index extends Base
{
    ...
    public function welcome()
    {
        ...
        $ajaxLogic->update_template('users'); // 升级前台用户中心的模板文件
        ...
        $ajaxLogic->admin_logic_special_addfields(); // 同步专题模型内置的附加表字段(v1.5.1节点去掉)
        ...
        $ajaxLogic->admin_logic_users_parameter(); // 纠正用户属性的内置手机号码和邮箱地址(v1.5.1节点去掉)
        $ajaxLogic->admin_logic_users_download(); // 根据下载模型自动开启用户中心的【我的下载】(v1.5.1节点去掉)
    ...
    private function contentTotalList()
    {
        ...
        foreach ($quickentryList as $key => $val) {
            ...
            else if ($code == 'Member@users_index@') // 用户
            {
                $map = [
                    'lang'    => $this->admin_lang,
                    'is_del'    => 0,
