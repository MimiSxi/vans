...
class Level extends Base
{
    ...
    private function GetAddMoneyData($UsersTypeData = array(), $pay_method = 'balance', $status = 2, $details = '')
    {
        ...
        $details = '用户当前级别为【'.$this->users['level_name'].'】，使用'.$pay_method_new.'支付【 '.$UsersTypeData['type_name'].'】，支付金额为'.$UsersTypeData['price'];
    ...
    private function GetUpMoneyData($data = array(), $pay_method = 'balance')
    {
        ...
        $details = '用户当前级别为【'.$this->users['level_name'].'】，使用'.$pay_method_new.'支付【 '.$data['type_name'].'】，支付金额为'.$data['price'];
    ...
    private function GetUpUsersData($data = array(), $balance = false)
    {
        ...
        // 判断是否需要追加天数，maturity_code在Base层已计算，1表示终身用户天数
        if (1 != $this->users['maturity_code']) {
            // 判断是否到期，到期则执行，3表示用户在期限内，不需要进行下一步操作
        ...
        // // 若用户已经有级别的天数和价格都低于用户将要购买的级别则追加用户当前选择的用户级别
